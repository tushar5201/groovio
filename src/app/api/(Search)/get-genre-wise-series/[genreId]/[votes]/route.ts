...
export async function GET(request: NextRequest) {
    ...
    try {
        ...
        const genreId = parts[parts.length - 2];
        const votes = parts[parts.length - 1]; // gets the [genreId] from /api/get-genre-wise-movie/[genreId]

        const res = await fetch(
            `https://api.themoviedb.org/3/discover/tv?include_adult=true&include_video=false&language=en-US&page=1&sort_by=popularity.desc&with_genres=${genreId}&vote_average.gte=${votes}`,
            {
                headers: {
